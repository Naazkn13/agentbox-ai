...
if [[ "$FILE_PATH" == *"test_"* ]] || [[ "$FILE_PATH" == *"_test."* ]] || [[ "$FILE_PATH" == *".test."* ]] || [[ "$FILE_PATH" == *".spec."* ]]; then
  ...
  if [ "$EXT" = "py" ]; then
    python3 -m py_compile "$FILE_PATH" 2>&1 && echo "✓ syntax ($FILE_PATH)" || echo "✗ syntax ($FILE_PATH)"
  fi
  exit 0
fi

# ── Run quality gates via Python ─────────────────────────────────────────────
OUTPUT=$(python3 "$AGENTKIT_HOME/workflow/quality_gates.py" "$FILE_PATH" 2>&1)
EXIT_CODE=$?
...
if [ $EXIT_CODE -eq 0 ]; then
  python3 "$AGENTKIT_HOME/workflow/enforcer.py" on-edit "$FILE_PATH" 2>/dev/null || true
fi
